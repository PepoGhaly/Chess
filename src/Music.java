...
public class Music  {
    static Clip c,move,check,killPiecesSound;
      static String filePath1,filePath2,filePath3,filePath4;
    //public  Music() {
        public void sound(){
            try {
                filePath1 = "src\\soundtrack.wav";
   ...
   static public void setKillPiecesSound(){
        try {
            filePath4 = "src\\ya wala ya za3em.wav";
            File musicpath4 = new File(filePath4);
            if (musicpath4.exists()) {
                AudioInputStream input = AudioSystem.getAudioInputStream(musicpath4);
                killPiecesSound = AudioSystem.getClip();
                killPiecesSound.open(input);
                killPiecesSound.start();
            } else {
                System.out.println("Music File is not exist.....");
            }
        }

...
public class board {
    ...
    public board() {
        for (int i=0;i<8;i++){
            for (int j=0;j<8;j++){
                Draw[i][j]=new Point(378+((i)*72),68+((j)*72));
            }
        }
    }
    ...
    public static void killPiece() {
        ...
        if (Pieces.secondValue < 0 && Pieces.secondValue > -9) {
            if (( (Computer.moving|| ProtectKing.moving) && !Pieces.isMoved )||(!Computer.moving&&!ProtectKing.moving )&&( !Pieces.isMoved || Game.p1.x == 20))  {

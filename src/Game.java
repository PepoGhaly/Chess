...
public class Game extends MouseAdapter implements MouseMotionListener {
    ...
    static Display deeply = new Display();
    ...
    JLabel Exit_l;
    ImageIcon Exit=new ImageIcon(getClass().getResource("\\Buttons\\Exi.png"));
    ...
    public Game() {
        ...
        if(Settings.themecount==1) {
            Display.Game.setContentPane(new JLabel(new ImageIcon(Display.img1)));
        }
        else if(Settings.themecount==2) {
            Display.Game.setContentPane(new JLabel(new ImageIcon(Display.img2)));
        }
        else if(Settings.themecount==3) {
            Display.Game.setContentPane(new JLabel(new ImageIcon(Display.img3)));
        }
        for (int i = 0; i < 8; i++) {
            for (int g = 0; g < 8; g++) {
                //Pawn
                if (board.piece[i][g] == 1 || board.piece[i][g] == -1 || board.piece[i][g] == -2|| board.piece[i][g] == -3|| board.piece[i][g] == -4
                    ...
                    Display.Game.add(pawn[pwn_cnt++].label);
                }
                ...
                //Rock
                    ...
                    Display.Game.add(rock[rck_cnt++].label);
                }
                ...
                //Knight
                    ...
                    Display.Game.add(knight[knight_cnt++].label);
                }
                //bishop
                    ...
                    Display.Game.add(bishop[bishop_cnt++].label);
                }

                //Queen
                else if (board.piece[i][g] == 12 || board.piece[i][g] == -12) {
                    queen[qn_cnt] = new Queen(board.piece[i][g], g, i);
                    Display.Game.add(queen[qn_cnt++].label);
                }

                //King
                else if (board.piece[i][g] == 13 || board.piece[i][g] == -13) {
                    king[kng_cnt] = new King(board.piece[i][g], g, i);
                    Display.Game.add(king[kng_cnt++].label);
                }
            }
        }
        Exit_l=new JLabel(Exit);
        Exit_l.setBounds(18,21,66,66);
        Exit_l.addMouseListener(this);
        Display.Game.add(Exit_l);

        Time1=new CoolDownTimer(new Point(1062,622));
        Time2=new CoolDownTimer(new Point(113,622));
        if (Settings.count==1){
            Time1.counterLabel.setText("10:00");
            Display.Game.add(Time1.counterLabel);
            Time1.minute=10;
            Time1.second=0;
            Time2.counterLabel.setText("10:00");
            Display.Game.add(Time2.counterLabel);
            Time2.minute=10;
            Time2.second=0;
        }
        ...
        Name1.setBounds(730,710,500,65);
        name2.setBounds(84,710,500,65);
        Name1.setHorizontalAlignment(JLabel.LEFT);
        name2.setHorizontalAlignment(JLabel.RIGHT);
        Name1.setFont(font);
        name2.setFont(font);
        Display.Game.add(Name1); Display.Game.add(name2);

        //deeply.setSize(1289, 758);

        Display.Game.addMouseListener(this);
        Display.Game.addMouseMotionListener(this);
    ...
    public void mousePressed (MouseEvent e ) {
        ...
        if (is_point && p2.x == 20) {
            ...
            if (p1.x > -1 && p1.x < 8 && p1.y > -1 && p1.y < 8) {
                ...
                Display.Game.add(box_Border_l2);
    ...
    public void ValidCircle(){
        for (int i=0;i<Pieces.counter;i++){
            ...
            Display.Game.add(validationCircles[i].circle_l).setBounds(board.Draw[Pieces.ValidPlacesY[i]][Pieces.ValidPlacesX[i]].x,board.Draw[Pieces.ValidPlacesY[i]][Pieces.ValidPlacesX[i]].y,72,72);
        }
    }
}
